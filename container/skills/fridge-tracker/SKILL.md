...
Maintain an estimated view of what the household currently has at home, based on delivery history and category-based
consumption rates. The state is stored in `/workspace/group/grocery/fridge-state.md`.
...
1. **Calibrated rate** — Check `## Calibrated Rates` section in `fridge-state.md`. If the specific product has a
   calibrated rate based on actual reorder intervals, use it. These are the most accurate.
2. **Custom rate** — Check `## Custom Rates` section in `fridge-state.md`. If the user manually corrected an item's
   consumption, use their override.
...
Last delivery: YYYY-MM-DD Last updated: YYYY-MM-DD Household: N adults, M children
...
- The user hasn't ordered in longer than usual (consumption estimates become less reliable after 2x the normal order
  interval)
...
- Adjust the consumption rate for that item category in future estimates (note in a `## Custom Rates` section at the
  bottom of the file)
